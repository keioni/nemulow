...
ファイルの先頭は `# nemulow v1` という行で始める。

## メタデータ

バージョン指定から `# article` という行までのリストは、メタデータとして扱われる。<br>
空行は無視される。

必須のメタデータのうち、記事の題名やタイムスタンプは、ソースファイルの名前から取得する。

例:

```plain0text
20250612_ねこ以上に可愛い生き物は存在するか？.md
```

この場合、記事名は「ねこ以上に可愛い生き物は存在するか？」となり、記事の日付は 2025/06/12 となる。
...
出力する html のファイル名。拡張子を除く。

「ねこ以上に可愛い生き物は存在するか？」をそのまま日本語として出力すると、URLエンコードされることになるので、一覧性が悪くなる。そのため、英語で付ける必要がある。

これには `.html` などの拡張子は付けない。
...
カテゴリを指定する。カテゴリは、表示名、内部名のどちらでもよい。カテゴリについては本ページの項目を参照のこと。

## 本文

いくつかのヘッダ(`#`)をディレクティブとして使う

### summary

`# summary` はOGPの description などに反映される。

### article

`# article` から始まるのが本文である。

ここから `#see more` までは、トップページや一覧ページなどで表示される。

つまり「続きを読む」ボタンを押すところまで表示する。
...
この場合、複数の記事を含む html ファイルでは `# see　more` の前の行までが使われる。この箇所に「続きを読む」といった、個別記事へのリンクを作る。

個別記事のファイルでは `#see　more` を含むすべてを表示する。
